...
public class DesktopApp {
    ...
    private JPanel createHistoryPanel(String filterType) {
        ...
        JToolBar toolBar = new JToolBar();
        JButton refreshBtn = new JButton(translate("REFRESH"));
        refreshBtn.addActionListener(e -> loadHistoryTable(panel, filterType));
        toolBar.add(refreshBtn);
        panel.add(toolBar, BorderLayout.NORTH);
        ...
        // Store model and filterType in the panel
        panel.putClientProperty("historyModel", model);
        panel.putClientProperty("filterType", filterType);

        loadHistoryTable(panel, filterType);

        return panel;
    }

    private void loadHistoryTable(JPanel panel, String filterType) {
        ...
        SwingWorker<Void, Void> worker = new SwingWorker<Void, Void>() {
            ...
            @Override
            protected void done() {
                DefaultTableModel model = (DefaultTableModel) panel.getClientProperty("historyModel");
                if (model != null) {
                    model.setRowCount(0);
                    if (historyData != null) {
    ...
    private void createUser(String fullName, String username, String password, String role, String department, JDialog dialog) {
        ...
        SwingWorker<Void, Void> worker = new SwingWorker<Void, Void>() {
            ...
            protected void done() {
                if (statusLabel != null) statusLabel.setText(translate("Ready"));
                if (success) {
                    JOptionPane.showMessageDialog(dialog, translate(message),
                            translate("Success"), JOptionPane.INFORMATION_MESSAGE);
                    dialog.dispose();
                    loadUsers();
                } else {
                    JOptionPane.showMessageDialog(dialog, translate("Failed: ") + message,
                            translate("Error"), JOptionPane.ERROR_MESSAGE);
                }
            }
        };
